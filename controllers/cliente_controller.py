...
cliente_dp = Blueprint('cliente',__name__,url_prefix="/clientes")

@cliente_dp.route("/")
def index():
    #recupera todos los registros de la tabla clientes en forma de objeto
    clientes = Cliente.get_all()
    return cliente_view.list(clientes)

@cliente_dp.route("/create", methods=['GET','POST'])
def create():
...
@cliente_dp.route("/edit/<int:id>", methods=['GET','POST'])
def edit(id):
...
@cliente_dp.route("/delete/<int:id>")
def delete(id):
    cliente = Cliente.get_by_id(id)
    cliente.delete()
    return redirect(url_for('cliente.index'))